...
crew.addEventListener("click", () => {
  console.log("click");
  if (
    crewDrop.classList.contains("crew_drop_on") ||
    crewDrop.classList.contains("crew_drop_visible")
  ) {
    crewDrop.classList.add("crew_drop_off");
    crewDrop.classList.remove("crew_drop_on");
    crewDrop.classList.remove("crew_drop_visible");
  } else {
...
crewDropList.forEach((v) => {
  ...
  v.addEventListener("mouseleave", (e) => {
    if (e.target.innerText === "Partnership") {
      v.style.color = "#3A2053";
    } else {
      v.style.color = "#CCCCCC";
    }
